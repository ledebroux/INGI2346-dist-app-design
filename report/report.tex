...
As specified above, the use of a header allows the server (reap. the client) to read the exact right number of bytes specified in the header. More precisely, the server expects to receive headers, specifying the command to run.
...
\begin{itemize}
    ...
    \item message cosntaining the last packet.
\end{itemize}
...
The size for the packets is set at 1072 as it corresponds to two time the maximum segment size that ipv4 host are required to be able to handle.
